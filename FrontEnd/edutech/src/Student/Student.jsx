...
const Student = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await axios.get('http://localhost:5050/viewstudent');
  ...
  const postApi = async () => {
    try {
      await axios.post('http://localhost:5050/addstudent', formData);
      fetchData();
      setFormData({
        student_id: '',
        student_name: '',
  ...
  const updateApi = async () => {
    try {
      await axios.patch(`http://localhost:5050/updatestudent/${formData.student_id}`, formData);
      fetchData();
      setFormData({
        student_id: '',
        student_name: '',
              ...
              {loading ? (
                ...
                currentItems.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td className="px-5 py-3 border-b border-blue-400 bg-white text-lg">
                      <p className="text-black whitespace-no-wrap">{item.student_id}</p>
                    </td>
                    <td className="px-5 py-3 border-b border-blue-400 bg-white text-lg">
                      <p className="text-black whitespace-no-wrap">{item.student_name}</p>
                    </td>
                    <td className="px-5 py-3 border-b border-blue-400 bg-white text-lg">
                    ...
                    <td className="px-5 py-3 border-b border-blue-400 bg-white text-lg">
                      <p className="text-black whitespace-no-wrap">{item.education}</p>
                    </td>
      ...
      {showModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
          <div className="bg-white p-8 rounded-lg w-1/3 relative">
            ...
            <h2 className="text-lg font-semibold mb-4">{isUpdating ? 'Update Student' : 'Add New Student'}</h2>
